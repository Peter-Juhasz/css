...
public class ClassReferencesFinder(string name, Action<SourceSpan> found) : SyntaxLocatorWalker
{
    public override void Visit(ClassSelectorSyntax node)
    {
        ...
        {
            found(new(Consumed + node.DotToken.Width, node.NameToken.Width));
        }
    ...
    {
        if (node.NameToken.Value.Equals("class", StringComparison.OrdinalIgnoreCase) && node.OperatorToken?.Text == "~=" && node.ValueToken?.Value == name)
        {
            found(new(Consumed +
