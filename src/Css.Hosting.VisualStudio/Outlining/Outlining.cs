...
internal sealed class CssOutliningTaggerProvider : ITaggerProvider
{
    public ITagger<T> CreateTagger<T>(ITextBuffer buffer) where T : ITag
    {
        return buffer.Properties.GetOrCreateSingletonProperty(
            creator: () => new OutliningTagger(buffer)
        ) as ITagger<T>;
    }


    private sealed class OutliningTagger : ITagger<IOutliningRegionTag>
    {
        ...
        private static IReadOnlyList<ITagSpan<IOutliningRegionTag>> GetTags(ITextSnapshot snapshot, SyntaxTree syntaxTree)
        {
            var tags = new List<ITagSpan<IOutliningRegionTag>>();
            var walker = new OutliningSyntaxWalker(found =>
            {
                var span = new SnapshotSpan(snapshot, found.Position, found.Length);
                tags.Add(new TagSpan<OutliningRegionTag>(span, new OutliningRegionTag("...", span.GetText())));
            });
        ...
        public IEnumerable<ITagSpan<IOutliningRegionTag>> GetTags(NormalizedSnapshotSpanCollection spans)
        {
            ...
            return Array.Empty<TagSpan<IOutliningRegionTag>>();
        }

        public event EventHandler<SnapshotSpanEventArgs> TagsChanged;

        private record struct CacheEntry(ITextVersion Version, IReadOnlyList<ITagSpan<IOutliningRegionTag>> Tags);
    }
}
